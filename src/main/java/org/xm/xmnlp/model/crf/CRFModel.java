...
public class CRFModel implements ICacheAble {
    ...
    public static CRFModel loadTxt(String path, CRFModel instance) {
        ...
        // 先尝试从bin加载
        if (CRFModel.load(ByteArray.createByteArray(path + Static.BIN_EXT))) return CRFModel;
        ...
        // 缓存bin
        try {
            logger.info("开始缓存" + path + Static.BIN_EXT);
            DataOutputStream out = new DataOutputStream(new FileOutputStream(path + Static.BIN_EXT));
            CRFModel.save(out);
            out.close();
        } catch (Exception e) {
            logger.warn("在缓存" + path + Static.BIN_EXT + "时发生错误" + TextUtil.exceptionToString(e));
        }
    ...
    public boolean load(ByteArray byteArray) {
        ...
        try {
            ...
            logger.warn("缓存载入失败，可能是由于版本变迁带来的不兼容。具体异常是：\n" + TextUtil.exceptionToString(e));
            return false;
        }
    ...
    public static CRFModel load(String path) {
        CRFModel model = loadBin(path + Static.BIN_EXT);
