    ...
    public static class LineIterator implements Iterator<String> {
        ...
        public LineIterator(String path) {
            try {
                ...
                logger.warn("文件" + path + "不存在，接下来的调用会返回null" + TextUtil.exceptionToString(e));
                bw = null;
            } catch (IOException e) {
                logger.warn("在读取过程中发生错误" + TextUtil.exceptionToString(e));
                bw = null;
            }
        }
        ...
        public void close() {
            ...
            try {
                ...
                logger.warn("关闭文件失败" + TextUtil.exceptionToString(e));
            }
            return;
        }
